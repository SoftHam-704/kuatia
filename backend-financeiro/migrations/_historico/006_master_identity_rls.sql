...
AS $$
  SELECT EXISTS (
    ...
    WHERE ut.tenant_id = p_tenant_id
      AND u.master_usuario_id = NULLIF(current_setting('app.user_id', true), '')::INTEGER
...
CREATE POLICY tenant_user_membership ON public.usuarios_tenants
  USING (
    ...
    AND EXISTS (SELECT 1 FROM public.usuarios u WHERE u.id = usuario_id AND u.master_usuario_id = NULLIF(current_setting('app.user_id', true), '')::INTEGER)
  )
  WITH CHECK (
    tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT
    AND EXISTS (SELECT 1 FROM public.usuarios u WHERE u.id = usuario_id AND u.master_usuario_id = NULLIF(current_setting('app.user_id', true), '')::INTEGER)
...
CREATE POLICY tenant_admin_consolidated_reports ON public.reportes_consolidados
  ...
  WITH CHECK (
    tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::BIGINT
    AND EXISTS (SELECT 1 FROM public.usuarios u WHERE u.id = solicitado_por AND u.master_usuario_id = NULLIF(current_setting('app.user_id', true), '')::INTEGER)
